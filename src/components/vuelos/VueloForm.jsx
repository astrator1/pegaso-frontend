        ...
        <div className="grid gap-4 py-2">
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              ...
              <Select value={form.matricula} onValueChange={(v) => set("matricula", v)}>
                ...
                <SelectContent>
                  {aeronaves.filter((a) => !a.retirada || a.matricula === form.matricula).map((a) => (
                    <SelectItem key={a.id} value={a.matricula}>{a.matricula}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid gap-2">
            ...
            <Select value={form.bateria} onValueChange={(v) => set("bateria", v)}>
              ...
              <SelectContent>
                <SelectItem value={null}>Sin asignar</SelectItem>
                {baterias.filter((b) => b.estado !== "Desechada").sort((a, b) => naturalCompare(a.numero_asignado, b.numero_asignado)).map((b) => <SelectItem key={b.id} value={b.numero_asignado}>{b.numero_asignado} — {b.marca} {b.modelo}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="grid grid-cols-2 gap-4">
