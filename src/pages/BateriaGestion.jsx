...
export default function BateriaGestion() {
  ...
  const confirmDescarte = async () => {
    ...
    const fechaSinGuiones = fechaBaja.replaceAll("-", "");
    const numHist = `${fechaSinGuiones}BAT${descartando.numero_asignado}`;
  ...
  const recuperar = async (b) => {
    ...
    if (!numHist) return;
    const numOriginal = numHist.slice(numHist.indexOf("BAT") + 3);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
                    ...
                    <div><span className="text-slate-400 block text-xs">Horas vuelo</span>{formatDuration(horas)}</div>
                  </div>
                  {(() => {
                  ...
                  <div className="mt-4 flex gap-2 flex-wrap">
                    ...
                    <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700" onClick={() => openDescartar(it)}><Ban className="w-3.5 h-3.5 mr-1" /> Descartar</Button>
                  </div>
                </motion.div>
              );
            })}
        ...
        {historico.length > 0 && (
          <div className="mt-10">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-semibold text-slate-900">Histórico de baterías desechadas</h2>
            </div>
      ...
      <Dialog open={!!descartando} onOpenChange={(v) => !v && setDescartando(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Descartar batería</DialogTitle></DialogHeader>
          <p className="text-slate-600 text-sm">Se va a desechaar la batería <strong>{descartando?.numero_asignado}</strong> ({descartando?.marca} {descartando?.modelo}). El número asignado quedará libre para reutilizarlo en una nueva batería. Los vuelos y mantenimientos asociados pasarán al número histórico para conservar la trazabilidad.</p>
          <div className="grid gap-2">
            <Label>Fecha de baja</Label>
            <Input type="date" value={fechaBaja} onChange={(e) => setFechaBaja(e.target.value)} />
          </div>
          {descartando && fechaBaja && (
            <p className="text-sm text-slate-500 bg-slate-50 rounded-lg p-3">Número histórico generado: <span className="font-mono font-semibold text-slate-900">{fechaBaja.replaceAll("-", "")}BAT{descartando.numero_asignado}</span></p>
          )}
          <DialogFooter>
            <Button variant="outline" onClick={() => setDescartando(null)}>Cancelar</Button>
            <Button onClick={confirmDescarte} className="bg-red-600 hover:bg-red-700">Descartar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
